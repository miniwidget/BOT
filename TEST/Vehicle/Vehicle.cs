...
namespace TEST
{
    class Vehicle : InfinityBase
    {
        internal void ac130()
        {
            ...
            var Ori = player.Origin;
            //var inimapOrigins = Call < Entity > ("getEntArray", "minimap_corner", "targetname" );
            Vector3 ac130Origin = new Vector3(0, 0, 0);

            //if ( miniMapOrigins.size )
            //ac130Origin = player.Origin; //maps\mp\gametypes\_spawnlogic::findBoxCenter( miniMapOrigins[0].origin, miniMapOrigins[1].origin );

            Entity level_ac130 = Call<Entity>("spawn", "script_model", ac130Origin);
            level_ac130.Call("setModel", "c130_zoomrig");
            level_ac130.SetField("angles", new Vector3(0, 115, 0));
            //level_ac130.owner = undefined;
            //level_ac130.thermal_vision = "ac130_thermal_mp";
            //level_ac130.enhanced_vision = "ac130_enhanced_mp";

            // used for debug printing
            level_ac130.SetField("targetname", "ac130rig_script_model");
            //level_ac130.Call("hide");


            Entity objModel = Call<Entity>("spawnPlane", player, "script_model", player.Origin + new Vector3(0, 0, 30000), "compass_objpoint_ac130_friendly", "compass_objpoint_ac130_enemy");
            objModel.Call("notSolid");
            objModel.Call("linkTo", level_ac130, "tag_player", new Vector3(0, 80, 32), new Vector3(0, -90, 0));
            ...
            player.Call("PlayerLinkWeaponviewToDelta", level_ac130, "tag_player", 1, 35, 35, 35, 35);
            player.Call("setPlayerAngles", level_ac130.Call<Vector3>("getTagAngles", "tag_player"));
            ...
            Entity ac130model = Call<Entity>("spawn", "script_model", level_ac130.Call<Vector3>("getTagOrigin", "tag_player"));
            ac130model.Call("setModel", "vehicle_ac130_coop");
            ac130model.Call("linkTo", level_ac130, "tag_player", new Vector3(0, 80, 32), new Vector3(-25, 0, 0));
            ...
            level_ac130.Call("rotateyaw", level_ac130.GetField<Vector3>("angles").Y + rampupDegrees, rotateTime, rotateTime, 0);
            Print(ac130model.Origin + "//" + player.Origin + "//" + objModel.Origin + "//" + level_ac130.Origin);
            //ac130model.Call("moveTo", new Vector3(player.Origin.X + 10000, player.Origin.Y + 1000, level_ac130.Origin.Z), 5, 0, 0);// (destPoint, 40.0, 0.0, 0.0 );
            level_ac130.Call("moveTo", new Vector3(Ori.X, Ori.Y, Ori.Z), 15, 0, 0);// (destPoint, 40.0, 0.0, 0.0 );
            //(-1929.745, 4138.351, 4212.15)//(1069.5, -802.4, 2316.125)//(1069.5, -802.4, 32316.13)//(0, 0, 0)
        }
